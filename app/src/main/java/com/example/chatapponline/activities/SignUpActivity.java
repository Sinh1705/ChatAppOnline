...
public class SignUpActivity extends AppCompatActivity {
    ...
    private void signUp() {
        loading(true);
        //lấy ra dữ liệu từ editext và input vào firestore
    ...
    private String encodeImage(Bitmap bitmap) {
        int previewWith = 150;
        int previewHigh = bitmap.getHeight() * previewWith / bitmap.getWidth();
        Bitmap previewBitmap = Bitmap.createScaledBitmap(bitmap, previewWith, previewHigh, false);
    ...
    //sử dụng API mới trong android để chọn hình ảnh từ thư viện của người dùng và xử lý ảnh
    private final ActivityResultLauncher<Intent> pickImage = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(),
            result -> {
                //kiểm tra dũ liệu có tồn tại không
                if (result.getResultCode() == RESULT_OK) {
                    if (result.getData() != null) {
                        //nếu dữ liệu tồn tại lấy ra đường dẫn hiện tại
                        Uri imageUri = result.getData().getData();
                        try {
                            //thiết lập bitmap cho imageview
                            //ẩn text
